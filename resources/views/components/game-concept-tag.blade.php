@php
    ...
    $onClick = "@click.stop=\"\$dispatch('show-game-concept', {class: '{$dataForInit['class']}', id: '{$dataForInit['id']}'}); scrollToModal('game-concepts');\"";
@endphp
<span
    class="tag leading-loose rounded-md px-1 py-0.5 mr-0.5 mb-0.5 border border-gray-600 whitespace-nowrap {{ $gameConcept->typeSlug() }} clickable" {!! $onClick !!}>
    {{ $prepend ?? '' }}
    @if($gameConcept->icon())
